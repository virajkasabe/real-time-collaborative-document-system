...
export default function RightSidebar({
  ...
  commentAnchorRange, setCommentAnchorRange, onCommentClick,
}) {
            ...
            <div style={{ flex: 1, overflowY: 'auto', paddingBottom: '16px' }}>
              {comments.map((comment) => (
                <div
                  ...
                  style={{ cursor: comment.anchorRange ? 'pointer' : 'default' }}
                >
                  <div className="comment-header">
                    <span className="comment-author">{comment.author}</span>
                    <span className="comment-time">{comment.time}</span>
                  </div>
                  {comment.anchorRange?.selectedText && (
                    <blockquote className="anchored-comment-quote">
                      "{comment.anchorRange.selectedText}"
                    </blockquote>
                  )}
            ...
            {commentAnchorRange && (
              <div style={{
                ...
                <span style={{ textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', flex: 1 }}>
                  📌 Anchored to: <strong>"{commentAnchorRange.selectedText}"</strong>
                </span>
                <button
                  type="button"
                  onClick={() => setCommentAnchorRange(null)}
