...
export default function Sidebar({ sidebarOpen, setSidebarOpen, sidebarCollapsed, setSidebarCollapsed }) {
  ...
  return (
    <aside 
      className={`transition-all duration-300 ease-in-out overflow-hidden h-full z-40 select-none text-left shrink-0 relative
        absolute md:static top-0 left-0
        bg-white dark:bg-[#0B0F19]
        border-r border-[#E5E7EB] dark:border-white/5
        flex flex-col
        ${sidebarOpen 
          ? (sidebarCollapsed ? 'w-[240px] md:w-[52px] translate-x-0 overflow-y-auto' : 'w-[240px] translate-x-0 overflow-y-auto') 
          : (sidebarCollapsed ? 'w-0 md:w-[52px] -translate-x-full md:translate-x-0' : 'w-0 -translate-x-full md:translate-x-0')
        }
      `}
    >
      {/* Top Branding & Main Navigation */}
      <div className={`flex-1 flex flex-col min-h-0 ${(sidebarOpen && !sidebarCollapsed) ? 'overflow-y-auto' : 'overflow-visible md:overflow-visible'}`}>
        {/* Header Branding */}
        <div className="h-14 flex items-center px-4 border-b border-gray-200 dark:border-gray-700/50 shrink-0 relative overflow-hidden">
          ...
          <img
            ...
            className={`absolute h-8 w-auto object-contain transition-all duration-300 left-4 ${
              (sidebarOpen && !sidebarCollapsed) ? 'opacity-100 scale-100' : 'opacity-0 scale-75 pointer-events-none'
            }`}
            style={{
              maxWidth: '140px',
            ...
            return (
              <NavLink
                ...
                className={`w-full flex items-center rounded-lg text-[13.5px] ${isActive ? 'font-semibold text-[#0D6EFD]' : 'font-medium text-[#6B7280] dark:text-[#94A3B8]/65 hover:text-[#081B3A] dark:hover:text-[#E5E7EB]'} transition-all duration-300 group relative h-10 px-3
                  ${(sidebarOpen && !sidebarCollapsed) ? 'justify-start gap-2.5' : 'justify-center'}
                  ${isActive 
      ...
      {/* Bottom Sidebar Menu */}
      <div className={`border-t border-[#E5E7EB] dark:border-white/10 bg-slate-50/50 dark:bg-[#060B14] shrink-0 ${(sidebarOpen && !sidebarCollapsed) ? 'p-3' : 'p-2'} ${(sidebarOpen && !sidebarCollapsed) ? 'overflow-y-auto' : 'overflow-visible md:overflow-visible'}`}>
      ...
      <div className="mt-auto border-t border-[#E5E7EB] dark:border-white/5 pt-1">
        <button
          onClick={() => setSidebarCollapsed(p => !p)}
          title={sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}
          className="w-full flex items-center justify-center p-2
            rounded-lg text-[#9CA3AF]
            hover:text-[#1a56db] hover:bg-[#f0f4ff]
            dark:hover:bg-[#0F172A] transition-colors cursor-pointer"
        >
          {sidebarCollapsed
