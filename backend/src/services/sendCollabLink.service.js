...
export const registerAndJoinCollab = async (
  documentName, inviterName, acceptLink, declineLink,
  recipientEmail, inviterEmail, recipientName = null, registrationLink
) => {
  try {
    ...
    const response = await apiInstance.transactionalEmails.sendTransacEmail({
      ...
      sender: { name: inviterName || 'Collaboration System', email: inviterEmail || senderEmail }
    });
...
export const joinCollab = async (
  documentName, inviterName, acceptLink, declineLink,
  userEmail = null, inviterEmail = null, loginLink
) => {
  try {
    ...
    const response = await apiInstance.transactionalEmails.sendTransacEmail({
      to: [{ email: userEmail || senderEmail, name: userEmail ? userEmail.split('@')[0] : 'Admin' }],
