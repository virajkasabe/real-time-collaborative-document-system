...
© ${new Date().getFullYear()} Collaboration Platform. All rights reserved.`
        ...
        return { success: true, data: response.data, message: 'Password reset email sent successfully' }
    } catch (error) {
        console.error('Error sending password reset email:', error.response?.data || error.message)
        return { success: false, error: error.response?.data || error.message, message: 'Failed to send password reset email' }
    }
}
