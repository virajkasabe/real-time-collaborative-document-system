...
const onlineUser = async (io, realTimeNotificationData, collabData, email) => {
  realTimeNotificationData.expiry = new Date(
    Date.now() + 15 * 60 * 1000
  ).toLocaleString();
  ...
  await setCollaboration(
    realTimeNotificationData.collabId,
    collabData,
    notificationExpiry
  );
};
    ...
    if (!accepter) {
      ...
      .json(
        new ApiResponse(
          200,
          {},
          "user not register then send invite link using the email"
        )
      );
    }
      ...
      return res
        ...
        .json(
          new ApiResponse(
            200,
            {},
            "user online then send invite link using Notification"
          )
        );
    }
      ...
      return res
        ...
        .json(
          new ApiResponse(
            200,
            {},
            "user offline send the link using email + pending notification"
          )
        );
    }
  } catch (error) {
